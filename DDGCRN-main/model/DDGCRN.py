...
class DGCRM(nn.Module):
    def __init__(self, args,node_num, dim_in, dim_out, cheb_k, embed_dim, num_layers=1):
        ...
        self.DGCRM_cells.append(DDGCRNCell(args,node_num, dim_in, dim_out, cheb_k, embed_dim))
        for _ in range(1, num_layers):
            self.DGCRM_cells.append(DDGCRNCell(args,node_num, dim_out, dim_out, cheb_k, embed_dim))
    ...
    def init_hidden(self, x, batch_size):
        init_states = []
        for i in range(self.num_layers):
            init_states.append(self.DGCRM_cells[i].init_hidden_state(x,batch_size))
...
class DDGCRN(nn.Module):
    def __init__(self, args):
        ...
        self.encoder1 = DGCRM(args,args.num_nodes, args.input_dim, args.rnn_units, args.cheb_k,
                              args.embed_dim, args.num_layers)
        self.encoder2 = DGCRM(args,args.num_nodes, args.input_dim, args.rnn_units, args.cheb_k,
                              args.embed_dim, args.num_layers)
        #predictor
    ...
    def forward(self, source, i=2):
        ...
        if i == 1:
            init_state1 = self.encoder1.init_hidden(source,source.shape[0])  # [2,64,307,64] 前面是2是因为有两层GRU
            ...
            init_state2 = self.encoder2.init_hidden(source2,source2.shape[0])   #[2,64,307,64] 前面是2是因为有两层GRU
