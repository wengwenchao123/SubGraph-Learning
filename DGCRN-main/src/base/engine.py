...
class BaseEngine():
    def __init__(self, device, model, dataloader, scaler, sampler, loss_fn, lrate, optimizer, \
        ...
        self._seed = seed
        self._meminfo = 0
        self._handle = pynvml.nvmlDeviceGetHandleByIndex(int(str(device)[5]))
    ...
    def train(self):
        ...
        wait = 0
        min_loss = np.inf
        self._meminfo = pynvml.nvmlDeviceGetMemoryInfo(self._handle)
        for epoch in range(self._max_epochs):
            t1 = time.time()
            mtrain_loss, mtrain_mape, mtrain_rmse = self.train_batch()
            t2 = time.time()

            meminfo = pynvml.nvmlDeviceGetMemoryInfo(self._handle)
            ...
            v3 = time.time()
            mtest_loss, mtest_mape, mtest_rmse = self.evaluate('test')
            v4 = time.time()
            ...
            message = 'Epoch: {:03d}, Train Loss: {:.4f}, Train RMSE: {:.4f}, Train MAPE: {:.4f}, GPU cost: {:.2f} GB, Train Time: {:.2f}s/epoch,LR: {:.4e}' \

            self._logger.info(message.format(epoch + 1, mtrain_loss, mtrain_rmse, mtrain_mape, (meminfo.used - self._meminfo.used) / 1024 ** 3, (t2 - t1), cur_lr))
            message = 'Epoch: {:03d}, Valid Loss: {:.4f}, Valid RMSE: {:.4f}, Valid MAPE: {:.4f}, Valid Time: {:.2f}s'
            self._logger.info(message.format(epoch + 1,mvalid_loss, mvalid_rmse, mvalid_mape, (v2 - v1)))
            message = 'Epoch: {:03d},  test Loss: {:.4f},  test RMSE: {:.4f},  test MAPE: {:.4f},  test Time: {:.2f}s'
            self._logger.info(message.format(epoch + 1,mtest_loss, mtest_rmse, mtest_mape, (v4 - v3)))
