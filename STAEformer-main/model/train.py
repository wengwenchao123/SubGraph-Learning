    ...
    for epoch in range(max_epochs):
        ...
        if (epoch + 1) % verbose == 0:
            print_log(
                ...
                "Train Time = %.4f s" % (train_time2-train_time1),
                "GPU Cost = %.2f GB" % ((meminfo2.used - meminfo1.used)/ 1024 ** 3),
                "Val Loss = %.4f" % val_loss,
                "Val Time = %.2f s" % (val_time2-val_time1),
                log=log,
            )
...
if __name__ == "__main__":
    ...
    handle = pynvml.nvmlDeviceGetHandleByIndex(args.gpu_num)
    ...
    elif dataset in ("PEMS03", "PEMS04", "PEMS07", "PEMS08","SD","GBA","GLA"):
    ...
    #这部分代码能够显示模型的每层的参数情况，但是不知道为什么会一直占用着GPU内存，所以注释掉
    # print_log(
    #     summary(
    #         model,
    #         [
