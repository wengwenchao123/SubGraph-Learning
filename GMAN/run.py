...
from model.model import GMAN as Network
...
args.add_argument('--dataset', default='PEMSD8', type=str)  # NYCBike2 NYCBike1 NYCTaxi
...
args.add_argument('--model', default='GMAN', type=str)
...
args.add_argument('--memory_node', default=config['model']['memory_node'], type=int)
args.add_argument('--topk', default=config['model']['topk'], type=int)
args.add_argument('--use_subgraph', default=config['model']['use_subgraph'], type=eval)
...
args.add_argument('--heads', default=config['model']['heads'], type=int)
args.add_argument('--dim', default=config['model']['dim'], type=int)
args.add_argument('--SE_dim', default=config['model']['SE_dim'], type=int)
#train
...
#load dataset
train_loader, val_loader, test_loader, SE, scaler = get_dataloader(args,
                                                               normalizer=args.normalizer,
                                                               tod=args.tod, dow=False,
                                                               weather=False, single=False)

#init model
model = Network(args.heads,args.dim,args.SE_dim,args.steps_per_day+args.steps_per_week,SE.to(args.device),args)
