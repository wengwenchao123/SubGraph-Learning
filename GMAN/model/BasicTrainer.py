...
class Trainer(object):
    def __init__(self, model, loss, optimizer, train_loader, val_loader, test_loader,
        ...
        self.batches_seen = 0
        self.meminfo = 0
        self.handle = pynvml.nvmlDeviceGetHandleByIndex(int(args.device[5]))
        #if not args.debug:
    ...
    def val_epoch(self, epoch, val_dataloader):
        ...
        with torch.no_grad():
            for batch_idx, (data, target) in enumerate(val_dataloader):
                ...
                label = target[..., :self.args.output_dim].clone().to(self.device)
    ...
    def test_epoch(self, epoch, test_dataloader):
        ...
        with torch.no_grad():
            for batch_idx, (data, target) in enumerate(test_dataloader):
                data = data.to(self.device)
                label = target[..., :self.args.output_dim].clone().to(self.device)
                # target[...,:self.args.output_dim] = self.scaler.transform(target[...,:self.args.output_dim]).to(self.device)
                target = target.to(self.device)
                output = self.model(data, target)
                if self.args.real_value:
                    output = self.scaler.inverse_transform(output)
    ...
    def train_epoch(self, epoch):
        ...
        for batch_idx, (data, target) in enumerate(self.train_loader):
            self.batches_seen += 1
            data = data.to(self.device)
            label = target[..., :self.args.output_dim].clone().to(self.device) # (..., 1)
            # target[..., :self.args.output_dim] = self.scaler.transform(target[..., :self.args.output_dim])
            target = target.to(self.device)
